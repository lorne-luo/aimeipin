...
import com.meidi.repository.GroupLaunchRepository;
import com.meidi.repository.OrderRepository;
...
public class WxPayTestController extends WxBaseController {
...
//            client.setParameter(ClientPNames.ALLOW_CIRCULAR_REDIRECTS, true);
     ...
     */
    @ResponseBody
    @RequestMapping(value = "/notifyUrlTest", method = RequestMethod.POST)
    public Map notifyUrlTest(HttpServletRequest request,
                             @RequestParam(value = "orderCode") String orderCode) {
        MdModel model = new MdModel(request);


        String transaction_id = MdCommon.getRandomString(16);
     ...
     * @return
     */
    @RequestMapping(value = "/payResultTest/{orderCode}", method = RequestMethod.GET)
    public ModelAndView payResultTest(HttpServletRequest request,
                                      @PathVariable String orderCode) {
        MdModel model = new MdModel(request);
        try {
            String transaction_id = MdCommon.getRandomString(16);//微信订单号
            Order order = orderRepository.findByOrderCode(orderCode);
            ...
            if ("SUCCESS".equals("SUCCESS")) {//支付成功
                ...
                if (order.getState() == 2) {//表示在回调时以处理
                    ...
                    //发消息
//                    WxTemplate.groupLaunch("1lbj4s7tocEy0RQWuuKPVqcYQjth5oVzTBrxDfIYLpp3W49fUbM_8bfxlN0pRj-I3LpdNheB0B3zsN1RYEhvucz2-fqAK55mwCnuQcaH1WZhsSdqyxZmo16PJd8JNPXmANNeAJAWJJ", order);
                }


            } else if ("NOTPAY".equals(1)) {//未支付
                order.setState(1);
            } else if ("PAYERROR".equals(2)) {//支付失败
                order.setState(3);
            }
            order = orderRepository.save(order);
            model.put("order", order);
...
//                        //拼团成功 给每个用户发消息
//                        WxTemplate.groupLaunchOk(wxTicket.getToken(), user.getWxOpenid(), order);
//                    }
                } else {
                    //成功参团
                    WxTicket wxTicket = wxTicketRepository.findByAppid(WX_APP_ID);
